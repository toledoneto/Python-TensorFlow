# agora vamos observer o efeito de ações passadas e tentar aprender bons movimentos futuros
...
outputs = tf.nn.sigmoid(logits)  # % de ir pra esq

probabilties = tf.concat(axis=1, values=[outputs, 1 - outputs])
action = tf.multinomial(probabilties, num_samples=1)

# transformando o tensor (obj da TF) para float para podermos usar no treinamento
...
################################################################
# func recompensa
# literatura: https://medium.com/@awjuliani/super-simple-reinforcement-learning-tutorial-part-2-ded33892c724
...
with tf.Session() as sess:
    ...
    for iteration in range(num_iterations):
        ...
        for game in range(num_game_rounds):
            ...
            # pega a primeira situação do jogo
            observations = env.reset()

            # permitir apenas n steps
            for step in range(max_game_steps):

                # recebe o gradiente e as ações
                action_val, gradients_val = sess.run([action, gradients], feed_dict={X: observations.reshape(1, num_inputs)})

                # realiza a ação
                observations, reward, done, info = env.step(action_val[0][0])

                # pega o gradiente e as ações atuais
                current_rewards.append(reward)
                current_gradients.append(gradients_val)

                # se o jogo acabar, quebra o laço
                if done:
                    break

            # cria uma lista com todas as recompensas
